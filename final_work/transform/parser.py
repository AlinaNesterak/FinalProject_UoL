...
@dataclass
class Source:
    """A manuscript or printed source for a work."""
    title: Optional[str] = None
    repository: Optional[str] = None
    shelfmark: Optional[str] = None
    source_type: Optional[str] = None   # "manuscript" | "print"


@dataclass
class Performance:
    """A documented performance of a work."""
...
class Incipit:
    ...
    # Parallel duration sequence, e.g. [8, 8, 4] (MEI @dur: 4=quarter, 8=eighth)
...
class Work:
    ...
    composer_uri: Optional[str] = None          # VIAF (primary authority)
    composer_wikidata: Optional[str] = None     # linked-data: Wikidata
    composer_musicbrainz: Optional[str] = None  # linked-data: MusicBrainz artist
    work_wikidata: Optional[str] = None         # linked-data: Wikidata (work-level, if known)
    work_musicbrainz: Optional[str] = None      # linked-data: MusicBrainz work (if known)
    imslp_url: Optional[str] = None             # linked-data: IMSLP score page (if known)
...
def _text(el: etree._Element | None) -> Optional[str]:
...
def _parse_incipit(work_el: etree._Element) -> Optional[Incipit]:
    ...
    for note in _findall(incip_el, ".//mei:note"):
        ...
        if pname and octave:
            ...
            dur = note.get("dur")
            inc.durations.append(int(dur) if dur and dur.isdigit() else 4)
...
def parse_file(path: str | Path) -> Work:
    ...
    cat_number = "?"
    opus = None
    # Identifier types that are not catalogue identifiers (linked-data
    # references living in the same pubStmt/identifier list).
    _NON_CATALOGUE_TYPES = {"OPUS", "WIKIDATA-WORK", "MUSICBRAINZ-WORK", "IMSLP"}
    for ident in _findall(root, ".//mei:pubStmt/mei:identifier"):
        itype = (ident.get("type") or "").upper()
        val = _text(ident)
        if itype == "OPUS":
            opus = val
        elif itype and itype not in _NON_CATALOGUE_TYPES and val:
            catalogue = itype
            cat_number = val
    ...
    # Composer (primary authority = VIAF, from the work element)
    comp = _find(work_el, "mei:composer")
    if comp is not None:
        work.composer = _text(comp)
        work.composer_uri = comp.get("auth.uri")

    # Composer linked-data identifiers (Wikidata, MusicBrainz), declared as
    # additional persName entries with different @auth values in the file
    # header's respStmt — only present where independently verified.
    for pn in _findall(root, ".//mei:respStmt/mei:persName[@role='composer']"):
        auth = pn.get("auth")
        uri = pn.get("auth.uri")
        if auth == "Wikidata":
            work.composer_wikidata = uri
        elif auth == "MusicBrainz":
            work.composer_musicbrainz = uri

    # Work-level linked-data identifiers (rare — only added where a specific
    # work, not just its composer, has a verified Wikidata/MusicBrainz entry)
    for ident in _findall(root, ".//mei:pubStmt/mei:identifier"):
        itype = ident.get("type") or ""
        if itype == "Wikidata-work":
            work.work_wikidata = _text(ident)
        elif itype == "MusicBrainz-work":
            work.work_musicbrainz = _text(ident)
        elif itype == "IMSLP":
            work.imslp_url = _text(ident)
    ...
    # Sources / manuscripts (from fileDesc/sourceDesc — mirrors MerMEId records)
    for src in _findall(root, ".//mei:sourceDesc/mei:source"):
        s = Source(
            title=_text(_find(src, ".//mei:titleStmt/mei:title")) or _text(_find(src, "mei:title")),
            repository=_text(_find(src, ".//mei:repository/mei:corpName")),
            shelfmark=_text(_find(src, ".//mei:identifier[@type='shelfmark']")),
            source_type=src.get("type"),
        )
        if s.title or s.repository or s.shelfmark:
            work.sources.append(s)

    # Performance history (work/history/eventList[@type='performances'])
    for ev in _findall(work_el, ".//mei:history//mei:eventList[@type='performances']/mei:event"):
        date_el = _find(ev, "mei:date")
        date_val = _text(date_el)
        if date_val is None and date_el is not None:
            date_val = date_el.get("isodate")
        perf = Performance(
            date=date_val,
            description=_text(_find(ev, "mei:desc")),
        )
        if perf.date or perf.description:
            work.performances.append(perf)

    # Narrative composition history (free-text <p> inside <history>, if present)
    hist_paras = _findall(work_el, ".//mei:history/mei:p")
    if hist_paras:
        work.history_note = " ".join(t for p in hist_paras if (t := _text(p)))
