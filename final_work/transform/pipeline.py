...
class PipelineReport(TypedDict):
    """Structured result of a pipeline run, used by the evaluation script
    and by anything reporting on transformation completeness/robustness."""
    works_loaded: int
...
CREATE TABLE IF NOT EXISTS works (
    ...
    incipit_durations TEXT,   -- JSON array of durations (for audio/notation)
    sources           TEXT,   -- JSON array of {title, repository, shelfmark, type}
    performances      TEXT,   -- JSON array of {date, description}
    history_note      TEXT,   -- narrative composition history (free text)
    composer_wikidata TEXT,   -- linked data: Wikidata composer URI
    composer_musicbrainz TEXT, -- linked data: MusicBrainz artist URI
    work_wikidata     TEXT,   -- linked data: Wikidata work URI (rare, verified only)
    work_musicbrainz  TEXT,   -- linked data: MusicBrainz work URI (rare, verified only)
    imslp_url         TEXT,   -- linked data: IMSLP free-score page (verified only)
    source_file       TEXT
);
...
def build_database(data_dir: str | Path, db_path: str | Path) -> PipelineReport:
    """
    ...
    for w in works:
        ...
        if w.incipit:
            ...
            durations = json.dumps(w.incipit.durations)

        sources_json = json.dumps([
            {"title": s.title, "repository": s.repository,
             "shelfmark": s.shelfmark, "type": s.source_type}
            for s in w.sources
        ])
        performances_json = json.dumps([
            {"date": p.date, "description": p.description}
            for p in w.performances
        ])
        ...
        conn.execute(
            ...
            (
                ...
                tempo, key, meter, pitches, durations,
                sources_json, performances_json, w.history_note,
                w.composer_wikidata, w.composer_musicbrainz,
                w.work_wikidata, w.work_musicbrainz, w.imslp_url, w.source_file,
            ),
        )
